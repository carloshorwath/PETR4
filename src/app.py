    ...
    if not st.session_state.production_done:
        ...
        if start_btn:
            ...
            try:
                ...
                # Primeiro tenta usar o JSON (Mais preciso)
                if json_path.exists():
                    st.info("Usando sincronia de alta precisão (JSON)...")
                    from utils import parse_json_transcription, calculate_durations_from_words

                    with open(json_path, "r") as f:
                        json_content = f.read()

                    word_data = parse_json_transcription(json_content)
                    images_data = calculate_durations_from_words(segments, word_data, audio_duration)

                # Se não tiver JSON, usa o SRT (Padrão)
                else:
                    st.info("Usando sincronia padrão (SRT)...")
